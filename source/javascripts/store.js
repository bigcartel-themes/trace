...
header_nav = document.querySelector('header');

function handleScroll() {
  function isStuck(el) {
    if (window.scrollY > 0) {
      return el.offsetTop - window.scrollY <= 0 ? true : false;
    }
    else {
      return false;
    }
  }
  isStuck(header_nav) ? header_nav.classList.add("stuck") : header_nav.classList.remove("stuck");
}
